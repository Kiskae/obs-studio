...
#define blog(level, format, ...) \
	blog(level, __FUNCTION__ ": " format, __VA_ARGS__)

void *bi_def_bitmap_create(int width, int height)
{
...
bool bi_def_bitmap_test_opaque(void *bitmap)
{
	return false;
}

unsigned char *bi_def_bitmap_get_buffer(void *bitmap)
{
	return (unsigned char*)bitmap;
}

void bi_def_bitmap_destroy(void *bitmap)
{
	bfree(bitmap);
}
...
static bool init_animated_gif(gs_image_file_t *image, const char *path)
{
	...
	if (get_full_decoded_gif_size(image) != max_size) {
		...
		uint64_t elapsed_time_ns, int loops)
{
	...
	for (;;) {
		...
		image->cur_time -= t;
		if (++new_frame == image->gif.frame_count) {
